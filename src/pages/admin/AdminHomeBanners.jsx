...
//     try {
//       await axios.patch(`admin/home-banners/${b.id}/`, { activo: !b.activo }, { headers: getAuthHeaders() });
//       await load();
//     } catch (e) {
...
//     try {
//       await axios.patch(`admin/home-banners/${b.id}/`, fields, { headers: getAuthHeaders() });
//       await load();
//     } catch (e) {
...
//     try {
//       await axios.delete(`admin/home-banners/${b.id}/`, { headers: getAuthHeaders() });
//       await load();
//     } catch (e) {
...
//                       placeholder="Título"
//                       onBlur={(e) => {
//                         if (e.target.value !== (b.titulo || "")) onSaveMeta(b, { titulo: e.target.value });
//                       }}
//                     />
//                     <input
//                       className="border rounded px-2 py-1"
...
//                     </button>
//                     <button onClick={() => move(b, -1)} className="px-3 py-1 border rounded">↑</button>
//                     <button onClick={() => move(b, +1)} className="px-3 py-1 border rounded">↓</button>
//                     <button onClick={() => onDelete(b)} className="px-3 py-1 border rounded text-red-600">Eliminar</button>
//                   </div>
//                 </div>
//               </div>
//             ))
...
import api, { ensureCsrfCookie, getCSRFTokenFromCookie } from "../../api/axiosConfig";
...
// Prefijo real de los endpoints del app "products"
const API_PREFIX = "products/";
const P = (s) => API_PREFIX + s;
...
export default function AdminHomeBanners() {
  ...
  // Helper común para headers con CSRF
  ...
  const load = useCallback(async () => {
    ...
    setErr("");
    try {
      const { data } = await api.get(P("admin/home-banners/"), { params: { slot } });
      setBanners(Array.isArray(data) ? data : data.results || []);
    } catch (e) {
      setErr("No se pudieron cargar los banners.");
      setBanners([]);
  ...
  useEffect(() => {
    // Antes de cargar, intentamos tener la cookie CSRF (no molesta si ya está)
    (async () => {
  ...
  const onUpload = async (e) => {
    ...
    if (form.file.size > MAX) {
      setErr("Tamaño máximo 3MB.");
      return;
    }
    const low = (form.file.name || "").toLowerCase();
    if (![".webp", ".jpg", ".jpeg", ".png"].some((ext) => low.endsWith(ext))) {
      setErr("Formatos permitidos: webp/jpg/jpeg/png.");
      return;
    }
    ...
    try {
      await ensureCsrfCookie(); // aseguramos cookie CSRF
      // importante: NO setear Content-Type (dejar que el browser ponga el boundary)
      await api.post(P("admin/home-banners/"), fd, { headers: csrfHeaders() });
      setForm({ file: null, titulo: "", link: "", orden: 0 });
      setMsg("Banner subido.");
      await load();
    } catch (e) {
  ...
  const onToggle = async (b) => {
    try {
      await ensureCsrfCookie();
      await api.patch(P(`admin/home-banners/${b.id}/`), { activo: !b.activo }, { headers: csrfHeaders() });
      await load();
    } catch (e) {
      setErr("No se pudo cambiar el estado.");
    }
  ...
  // ────────────── Guardar meta (titulo, link, orden) (PATCH)
  const onSaveMeta = async (b, fields) => {
    try {
      await ensureCsrfCookie();
      await api.patch(P(`admin/home-banners/${b.id}/`), fields, { headers: csrfHeaders() });
      await load();
    } catch (e) {
      setErr("No se pudo guardar.");
    }
  ...
  const onDelete = async (b) => {
    ...
    try {
      await ensureCsrfCookie();
      await api.delete(P(`admin/home-banners/${b.id}/`), { headers: csrfHeaders() });
      await load();
    } catch (e) {
      setErr("No se pudo eliminar.");
    }
