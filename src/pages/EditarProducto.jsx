...
//     try {
//       await axios.delete(`products/images/${imgId}/eliminar/`);
...
const EditarProducto = () => {
  ...
  // ⬆️ Subir nuevas imágenes (permite cualquier tipo)
  const subirImagenes = async (fileList) => {
    const fd = new FormData();
    fd.append('product', id);
    for (let f of fileList) {
    ...
    try {
      await axios.post('products/images/upload/', fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      getImagenes(); // recargar
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto', padding: 20 }}>
      ...
      {/* 📤 Subir nuevas imágenes */}
      <div style={{ marginBottom: 20 }}>
        <h3>Agregar nuevas imágenes (JPG/PNG/WebP, etc.)</h3>
        <input
          type="file"
          accept="image/*"
          multiple
