...
const ListadoProductos = () => {
  ...
  const fetchProductos = async () => {
    try {
      ...
      if (st === 403) {
        // autenticado pero sin perfil de socio
  ...
  useEffect(() => {
    // opcional: loguear la base para chequear que no duplique /api/
    try { console.log("[ListadoProductos] baseURL:", axios.defaults.baseURL); } catch {}
    fetchProductos();
  }, []);
  ...
  const eliminarProducto = async (id) => {
    ...
    try {
      ...
      console.error(err);
      const st = err?.response?.status;
      if (st === 401) return navigate("/login", { replace: true });
      if (st === 403) return navigate("/socio/crear-perfil", { replace: true });
      setMensaje('');
      setError(err.response?.data?.error || 'Error al eliminar el producto');
    }
  };
